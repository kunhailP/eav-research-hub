...
| E4 | 집단당 300문장 인간 감사의 왜곡 예측은 왜곡 순위를 맞힌다 (0.88 vs 1 − 정확도 0.15). 그러나 위험 판별·모델 선택에서 정확도보다 낫다는 것은 확인되지 않았다 | **탐색** (UK). PImPo 확증 대기 | `prereg-audit-f` · [docs/18](docs/18_prereg_audit_forecast_draft.md) | 사후 교정(DSL/PPI)이 아니라 전체 코딩 **전** 예측 |
| E5 | 급진우파 vs 주류(PImPo, 다국어)에서 오픈 모델은 동등한가 | **진행 중** | `prereg-v3-pimpo` · [docs/14](docs/14_prereg_pimpo_draft.md) | 같은 텍스트 크라우드 기준, 14개국 |
| E6 | 같은 텍스트 기준에서 현재 프런티어 모델도 동등한가 | **미정: 결정적 검정** | API 실험 (별도 진행) | — |
| E7 | 오류가 텍스트가 아니라 정당 평판(이름 단서)에서 오는가 | **미정** (정당명 가림·바꿈 무작위 실험 사전등록 예정) | — | Vallejo Vera & Driggers (2025) 확장: 다국어·정당가족, 비교 추정량으로 연결 |
| E8 | 같은 계열에서 모델이 커질수록 집단별 오류 차이가 줄어드는가, 오히려 커지는가 | **미정** (Qwen 0.5B–14B 사다리 사전등록 예정) | — | — |
...
| `prereg-v3-pimpo` | 2026-09-14 09:09 | [docs/14](docs/14_prereg_pimpo_draft.md) | 라벨링·분석 실행 중 |
...
| `prereg-audit-f` | 2026-09-14 11:19 | [docs/18](docs/18_prereg_audit_forecast_draft.md) | UK 탐색 완료, PImPo 확증 대기 |
...
| [12_red_team](docs/12_red_team.md) | 자체 점검, 검토자 반론, **개발·일탈 기록 D1–D41** |
| [13](docs/13_prereg_v3_draft.md) · [14](docs/14_prereg_pimpo_draft.md) · [16](docs/16_prereg_benoit_e_draft.md) · [18](docs/18_prereg_audit_forecast_draft.md) | 사전등록 (잠김) |
| [15](docs/15_uk_results_interpretation.md) · [17](docs/17_benoit_e_results_interpretation.md) | 결과 해석 |
...
3. **불리한 결과도 같은 비중으로 보고한다.** 우호적 결과(E3)와 비지지 결과(E4)도 원장에 그대로 싣는다.
